...
def transition_model(corpus, page, damping_factor):
    ...
    # check if the page has outbound links.
    dict_len = len(corpus.keys())
    pages_len = len(corpus[page])

    if len(corpus[page]) < 1:
        # no outgoing pages, randomly choosing from all possible pages.
        for key in corpus.keys():
            prop_dist[key] = 1 / dict_len

    else:
        # there are outgoing pages, calculating the layout.
...
def iterate_pagerank(corpus, damping_factor):
    ...
    for page in corpus:
        olddict[page] = 1 / pages_number

    # by repeatedly calculating new range values ​​based on all current range values.
    while True:
        for page in corpus:
            temp = 0
            for linking_page in corpus:
                ...
                if page in corpus[linking_page]:
                    temp += (olddict[linking_page] / len(corpus[linking_page]))
                # if page has no links, interpret it as having one link for every other page.
                if len(corpus[linking_page]) == 0:
                    temp += (olddict[linking_page]) / len(corpus)
            temp *= damping_factor
            temp += (1 - damping_factor) / pages_number

            new_dict[page] = temp

        difference = max([abs(new_dict[x] - olddict[x]) for x in olddict])
        if difference < 0.001:
            break
        else:
            olddict = new_dict.copy()
